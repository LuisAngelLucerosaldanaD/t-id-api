...
func newValidationRequestPsqlRepository(db *sqlx.DB, user *models.User, txID string) *psql {
	return &psql{
		DB:   db,
		user: user,
		TxID: txID,
...
func (s *psql) create(m *ValidationRequest) error {
	const psqlInsert = `INSERT INTO cfg.validation_request (client_id, max_num_validation, request_id, expired_at, user_id, status) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id, created_at, updated_at`
...
func (s *psql) update(m *ValidationRequest) error {
	date := time.Now()
	m.UpdatedAt = date
	const psqlUpdate = `UPDATE cfg.validation_request SET client_id = :client_id, max_num_validation = :max_num_validation, request_id = :request_id, expired_at = :expired_at, user_id = :user_id, status = :status, updated_at = :updated_at WHERE id = :id `
...
func (s *psql) getByID(id int64) (*ValidationRequest, error) {
	const psqlGetByID = `SELECT id , client_id, max_num_validation, request_id, expired_at, user_id, status, created_at, updated_at FROM cfg.validation_request WHERE id = $1 `
...
func (s *psql) getAll() ([]*ValidationRequest, error) {
	...
	const psqlGetAll = ` SELECT id , client_id, max_num_validation, request_id, expired_at, user_id, status, created_at, updated_at FROM cfg.validation_request `
...
func (s *psql) getByClientIDAndRequestID(clientIid int64, requestID string) (*ValidationRequest, error) {
	const psqlGetByClientID = `SELECT id , client_id, max_num_validation, request_id, expired_at, user_id, status, created_at, updated_at FROM cfg.validation_request WHERE client_id = %d and request_id = '%s' limit 1`
	mdl := ValidationRequest{}
